...
addLayer("a", {
	...
	getPerdim() {
		return nd(2).mul(nd(1.05).pow(hasUpgrade('ip',22)?1.5:1).pow(getBuyableAmount('a',32).add(hasUpgrade('di',32)?upgradeEffect('di',32):0)))
	},
	getMult() {
...
addLayer("bd", {
	...
	tabFormat: {
		...
		"Antimatter":{content:[["display-text",()=>{return `Antimatter gain:${format(temp.a.getAMGain)}
														...
														<br><br>
														Generic Dimension Multiplier:${format(temp.a.getMult)}
...
addLayer("di", {
	...
	effect() {
		...
		if (player.m.antimatter.gt(10)) x=x.mul(player.m.antimatter.log10().pow(3))
		return x
	},
	effectDescription() {
	...
	upgrades: {
		...
		32: {
			...
			description: "Get free galaxies based on distance",
			cost: new Decimal(1e16),
			effect:()=>(player.di.dist.add(1)).log10().root(2).div(10).add(1),
			currencyDisplayName: "Time cubes",
			currencyLocation:()=>{return player.di.points},
			canAfford:function() {return (this.currencyLocation||(function() {return player[this.layer]}))().gt(this.cost)},
			pay:function() {return (this.currencyLocation||(function() {return player[this.layer]}))().subEq(this.cost)},
			unlocked() {return hasUpgrade('di',31)}
		},
		33: {
			title: "Baryon asymmetry",
			description: "Unlock matter.",
			cost: new Decimal(1e16),
			effect:()=>1,
			currencyDisplayName: "Time cubes",
			currencyLocation:()=>{return player.di.points},
...
addLayer("m", {
	...
	getPerdim() {
		return nd(2).mul(nd(1.05).pow(hasUpgrade('ip',22)?1.5:1).pow(getBuyableAmount('m',32).add(hasUpgrade('di',32)?upgradeEffect('di',32):0)))
	},
	getMult() {
		let perDim=layers.m.getPerdim()
		return nd(1)
		.mul(buyableEffect('m',31))
		.mul(perDim.pow(getBuyableAmount('m',31).mul(temp.m.getPerdim)))
		.mul(temp.di.effect.root(hasUpgrade('m',12)?4:7))
		.mul(temp.di.effect.root(!hasUpgrade('m',11)?upgradeEffect('m',11):1))
		.div(1e20)
		.root(3)
	},
	doNotCallTheseFunctionsEveryTick:["maxAllDims"],
	getAMGain() {
		let perDim=temp.m.getPerdim
		//console.log(perDim+'')
		let genericMult=temp.m.getMult
		let amGain=player.m.firstDim.add(perDim.pow(getBuyableAmount('m',11))).mul(genericMult)
		if (amGain.gt("1e308")) amGain=amGain.root(2).mul(1e154)
		if (amGain.gt("1e1000")) amGain=amGain.root(4).mul(1e750)
		amGain=amGain.root(hasUpgrade('m',13)?2:3)
		return amGain
	},
	update(diff) {
		...
		//console.log(perDim+'')
		let genericMult=temp.m.getMult
		let amGain=temp.m.getAMGain;
		let y=nd(1.01).pow(diff);
		(['firstDim','secondDim','thirdDim','fourthDim','fifthDim','sixthDim','seventhDim','eighthDim','antimatter']).forEach((x)=>{
			player.m[x]=player.m[x].div(y)
		})
		  player.m.antimatter.addEq(amGain.mul(diff))
		  player.m.firstDim.addEq(player.m.secondDim.add(getBuyableAmount('m',12)).mul(diff).mul(perDim.pow(getBuyableAmount('m',12))).mul(genericMult))
		  player.m.secondDim.addEq(player.m.thirdDim.add(getBuyableAmount('m',13)).mul(diff).mul(perDim.pow(getBuyableAmount('m',13))).mul(genericMult))
		  player.m.thirdDim.addEq(player.m.fourthDim.add(getBuyableAmount('m',14)).mul(diff).mul(perDim.pow(getBuyableAmount('m',14))).mul(genericMult))
		  player.m.fourthDim.addEq(player.m.fifthDim.add(getBuyableAmount('m',21)).mul(diff).mul(perDim.pow(getBuyableAmount('m',21))).mul(genericMult))
		   player.m.fifthDim.addEq(player.m.sixthDim.add(getBuyableAmount('m',22)).mul(diff).mul(perDim.pow(getBuyableAmount('m',22))).mul(genericMult))
		 player.m.sixthDim.addEq(player.m.seventhDim.add(getBuyableAmount('m',23)).mul(diff).mul(perDim.pow(getBuyableAmount('m',23))).mul(genericMult))
		player.m.seventhDim.addEq(player.m.eighthDim.add(getBuyableAmount('m',24)).mul(diff).mul(perDim.pow(getBuyableAmount('m',24))).mul(genericMult))
	},
	tooltip:()=>`${format(player.m.antimatter)} matter`,
    color: "#7777ff",
	base:nd(2),
    requires: new Decimal(1e10), // Can be a function that takes requirement increases into account
    resource: "free tickspeed upgrades", // Name of prestige currency
	...
	doReset(resLayer) {
		if (resLayer=='m') return;
		layerDataReset('m',hasMilestone('di',5)?['upgrades']:[]);
		//setBuyableAmount(31,nd(7))
	},
	canBuyMax:()=>true,
    gainExp() { // Calculate the exponent on main currency from bonuses
	...
	tabFormat: {
		"Upgrades": {
			content:["main-display", ["display-text",()=>`(${format(player.m.points.mul(0.13750352374993502))} effective purchased dimensions)`], "prestige-button", ["display-text",()=>`You have ${format(player.m.antimatter)} matter`],"upgrades"],
		},
		"Dimensions": {
			content:["main-display", ["display-text",()=>`(${format(player.m.points.mul(0.13750352374993502))} effective purchased dimensions)`],
			"prestige-button", ["display-text",()=>`You have ${format(player.m.antimatter)} matter`], ["clickables", 100], "buyables",
			["display-text", "You are losing 1% of all non-bought dimensions and matter every second."],
			],
			unlocked: fn(true)
		},
	},
	...
	buyables: {
		...
		11: {
				...
				description: "Get a first dimension",
				cost(x=getBuyableAmount(this.layer,this.id)) {
					return new Decimal(10).mul(new Decimal(1e3).pow(x.pow(2)))
				},
				display() {
					return `${this.description}<br>Cost: <b>${format(this.cost())}</b><br>You have bought <b>${format(getBuyableAmount(this.layer,this.id))}</b> of this dimension<br>(${format(getBuyableAmount(this.layer,this.id).add(player.m.firstDim))} total)`
				},
				effect:()=>(nd(2).pow(getBuyableAmount('p',11))),
				currencyDisplayName: "MA",
				currencyLocation:()=>{return player.m.antimatter},
				canAfford:function(x=getBuyableAmount(this.layer, this.id)) {return this.currencyLocation().gte(this.cost(x))},
				buy:function() {
					(this.currencyLocation)().subEq(this.cost());
					setBuyableAmount(this.layer, this.id, getBuyableAmount(this.layer,this.id).add(1))
		...
		12: {
				...
				description: "Get a second dimension",
				cost(x=getBuyableAmount(this.layer,this.id)) {
					return new Decimal(100).mul(new Decimal(1e4).pow(x.pow(2)))
				},
				display() {
					return `${this.description}<br>Cost: <b>${format(this.cost())}</b><br>You have bought <b>${format(getBuyableAmount(this.layer,this.id))}</b> of this dimension<br>(${format(getBuyableAmount(this.layer,this.id).add(player.m.secondDim))} total)`
				},
				effect:()=>(nd(2).pow(getBuyableAmount('p',11))),
				currencyDisplayName: "MA",
				currencyLocation:()=>{return player.m.antimatter},
				...
				unlocked() {
					return getBuyableAmount("m",31).gt(0)
				},
		},
		13: {
				title: "Third Dimension",
				description: "Get a third dimension",
				cost(x=getBuyableAmount(this.layer,this.id)) {
					return new Decimal(1e4).mul(new Decimal(1e5).pow(x.pow(2)))
				},
				display() {
					return `${this.description}<br>Cost: <b>${format(this.cost())}</b><br>You have bought <b>${format(getBuyableAmount(this.layer,this.id))}</b> of this dimension<br>(${format(getBuyableAmount(this.layer,this.id).add(player.m.thirdDim))} total)`
				},
				effect:()=>(nd(2).pow(getBuyableAmount('p',11))),
				currencyDisplayName: "MA",
				currencyLocation:()=>{return player.m.antimatter},
				...
				unlocked() {
					return getBuyableAmount("m",31).gt(1)
				},
		},
		14: {
				title: "Fourth Dimension",
				description: "Get a fourth dimension",
				cost(x=getBuyableAmount(this.layer,this.id)) {
					return new Decimal(1e7).mul(new Decimal(1e6).pow(x.pow(2)))
				},
				display() {
					return `${this.description}<br>Cost: <b>${format(this.cost())}</b><br>You have bought <b>${format(getBuyableAmount(this.layer,this.id))}</b> of this dimension<br>(${format(getBuyableAmount(this.layer,this.id).add(player.m.fourthDim))} total)`
				},
				effect:()=>(nd(2).pow(getBuyableAmount('p',11))),
				currencyDisplayName: "MA",
				currencyLocation:()=>{return player.m.antimatter},
				...
				unlocked() {
					return getBuyableAmount("m",31).gt(2)
				},
		},
		21: {
				title: "Fifth Dimension",
				description: "Get a fifth dimension",
				cost(x=getBuyableAmount(this.layer,this.id)) {
					return new Decimal(1e9).mul(new Decimal(1e8).pow(x.pow(2)))
				},
				display() {
					return `${this.description}<br>Cost: <b>${format(this.cost())}</b><br>You have bought <b>${format(getBuyableAmount(this.layer,this.id))}</b> of this dimension<br>(${format(getBuyableAmount(this.layer,this.id).add(player.m.fifthDim))} total)`
				},
				effect:()=>(nd(2).pow(getBuyableAmount('p',11))),
				currencyDisplayName: "MA",
				currencyLocation:()=>{return player.m.antimatter},
				...
				unlocked() {
					return getBuyableAmount("m",31).gt(3)
				},
		},
		22: {
				title: "Sixth Dimension",
				description: "Get a sixth dimension",
				cost(x=getBuyableAmount(this.layer,this.id)) {
					return new Decimal(1e13).mul(new Decimal(1e10).pow(x.pow(2)))
				},
				display() {
					return `${this.description}<br>Cost: <b>${format(this.cost())}</b><br>You have bought <b>${format(getBuyableAmount(this.layer,this.id))}</b> of this dimension<br>(${format(getBuyableAmount(this.layer,this.id).add(player.m.sixthDim))} total)`
				},
				effect:()=>(nd(2).pow(getBuyableAmount('p',11))),
				currencyDisplayName: "MA",
				currencyLocation:()=>{return player.m.antimatter},
				...
				unlocked() {
					return getBuyableAmount("m",31).gt(4)
				},
		},
		23: {
				title: "Seventh Dimension",
				description: "Get a seventh dimension",
				cost(x=getBuyableAmount(this.layer,this.id)) {
					return new Decimal(1e18).mul(new Decimal(1e12).pow(x.pow(2)))
				},
				display() {
					return `${this.description}<br>Cost: <b>${format(this.cost())}</b><br>You have bought <b>${format(getBuyableAmount(this.layer,this.id))}</b> of this dimension<br>(${format(getBuyableAmount(this.layer,this.id).add(player.m.seventhDim))} total)`
				},
				effect:()=>(nd(2).pow(getBuyableAmount('p',11))),
				currencyDisplayName: "MA",
				currencyLocation:()=>{return player.m.antimatter},
				...
				unlocked() {
					return getBuyableAmount("m",31).gt(5)
				},
		},
		24: {
				title: "Eighth Dimension",
				description: "Get a eighth dimension",
				cost(x=getBuyableAmount(this.layer,this.id)) {
					return new Decimal(1e24).mul(new Decimal(1e15).pow(x.pow(2)))
				},
				display() {
					return `${this.description}<br>Cost: <b>${format(this.cost())}</b><br>You have bought <b>${format(getBuyableAmount(this.layer,this.id))}</b> of this dimension<br>(${format(getBuyableAmount(this.layer,this.id).add(player.m.eighthDim))} total)`
				},
				effect:()=>(nd(2).pow(getBuyableAmount('p',11))),
				currencyDisplayName: "MA",
				currencyLocation:()=>{return player.m.antimatter},
				...
				unlocked() {
					return getBuyableAmount("m",31).gt(6)
				},
		},
		31: {
				title: "Dimension Boost",
				description: "Get a new dimension[until dim8] and add 2 effective bought dimensions<br>[note:doesn't reset dimensions]",
				cost(x=getBuyableAmount(this.layer,this.id)) {
					return new Decimal(1e2).mul(new Decimal(2e2).pow(x.pow(3)))
				},
				display() {
					return `${this.description}<br>Requires: <b>${format(this.cost())}</b><br>You have <b>${format(getBuyableAmount(this.layer,this.id))}</b> of this upgrade`
				},
		...
		32: {
				...
				description: "Multiplier per dimension is increased by *1.05",
				cost(x=getBuyableAmount(this.layer,this.id)) {
					return new Decimal(1e140).mul(new Decimal(1e50).pow(x.pow(4)))
				},
				display() {
					return `${this.description}<br>Requires: <b>${format(this.cost())}</b><br>You have <b>${format(getBuyableAmount(this.layer,this.id))}</b> of this upgrade`
				},
	...
	upgrades: {
		...
		cols: 3,
		11: {
				title: "Matter Synergy",
				description: "Matter gets boosted by antimatter",
				cost: new Decimal("1e3"),
				currencyDisplayName: "MA",
				currencyLocation:()=>{return player.m.antimatter},
				canAfford:function() {return (this.currencyLocation||(function() {return player[this.layer]}))().gt(this.cost)},
				pay:function() {return (this.currencyLocation||(function() {return player[this.layer]}))().subEq(this.cost)},
				effect() {return player.a.antimatter.log10().pow(3)},
			},
		12: {
				title: "Weakened timewalls",
				description: "Time speed boost to matter is better",
		...
		13: {
				title: "Matter Boost",
				description: "Matter gain softcap is weaker",
				cost: new Decimal(1e45),
				effect:()=>temp.m.effect,
				currencyDisplayName: "MA",
				currencyLocation:()=>{return player.m.antimatter},
				canAfford:function() {return (this.currencyLocation||(function() {return player[this.layer]}))().gt(this.cost)},
				pay:function() {return (this.currencyLocation||(function() {return player[this.layer]}))().subEq(this.cost)},
		...
		21: {
				title: "Boosted Time",
				description: "Matter boosts time speed",
				cost: new Decimal(1e210),
				effect:()=>(player.m.antimatter.add(1)).root(50),
				currencyDisplayName: "MA",
				currencyLocation:()=>{return player.m.antimatter},
				canAfford:function() {return (this.currencyLocation||(function() {return player[this.layer]}))().gt(this.cost)},
				pay:function() {return (this.currencyLocation||(function() {return player[this.layer]}))().subEq(this.cost)},
		...
		22: {
				title: "Boosted matter III",
				description: "Matter boost formula is <b>SIGNIFICANTLY</b> better.<br>Getting Tickspeed resets nothing.",
				cost: new Decimal("1e425"),
				currencyDisplayName: "MA",
				currencyLocation:()=>{return player.m.antimatter},
				effect:()=>(player.m.antimatter.add(1).root(30)),
				canAfford:function() {return (this.currencyLocation||(function() {return player[this.layer]}))().gt(this.cost)},
				pay:function() {return (this.currencyLocation||(function() {return player[this.layer]}))().subEq(this.cost)},
	...
	effect() {
		let temp=player.m.antimatter.add(1).log10().pow(2)
		if (hasUpgrade("m",13)) temp=temp.mul(upgradeEffect('m',13))
		if (hasUpgrade("m",22)) temp=temp.mul(upgradeEffect('m',22))
		if (temp.gt(1) && player.m.unlocked) {return (temp)}
		else {return nd(1)}
	},
});