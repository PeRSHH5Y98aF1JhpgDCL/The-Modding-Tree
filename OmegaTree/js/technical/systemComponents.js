var systemComponents = {
	...
	'overlay-head': {
		...
		<br>
		<span v-if="canGenPoints()"  class="overlayThing">({{format(getPointGen())}}/sec)</span><br>
		<span v-if="getPointGen().gt(1e20)"  class="overlayThing">(softcapped^{{format(getPointGen().log10().max(1).logBase(20).max(0).floor())}})[^{{format(new Decimal(2).pow(getPointGen().log10().max(1).logBase(20).max(0).floor().neg()))}}]</span>
