addLayer("p", {
        ...
        exponent() { return (((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?0.75:0.5)*0.9 }, // Prestige currency exponent
        gainMult() { // Calculate the multiplier for main currency from bonuses
			...
			mult=mult.mul(new Decimal(2).pow(player.f.buyables[12]))
            return mult
        },
...
addLayer("r", {
    ...
    baseAmount() {return player.points}, // Get the current amount of baseResource
    type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
    exponent: 0.5*0.9, // Prestige currency exponent
    gainMult() { // Calculate the multiplier for main currency from bonuses
        mult = new Decimal(1)
		mult=mult.mul(new Decimal(2).pow(player.f.buyables[13]))
        return mult
    },
    gainExp() { // Calculate the exponent on main currency from bonuses
	...
	milestones: {
		0: {
			requirementDescription: "10,000 Rockets",
			done() { return player.r.best.gte(1e4) || hasAchievement("a", 41) || hasAchievement("a", 71) },
			effectDescription: "Keep Prestige Upgrades on reset.",
		},
		1: {
			requirementDescription: "1e8 Rockets",
			done() { return player.r.best.gte(1e8) || hasAchievement("a", 41) || hasAchievement("a", 71) },
			effectDescription: "Rocket effect applies to generators.",
		},
	},
	effect() {
		let r=player.r.points
		r=r.add(tmp.f.effect.mul(10))
		if (r.gt(10)) r=r.log10().mul(10)
		let eff=r.add(1).log(3)
		eff=eff.mul(tmp.f.effect.add(1))
		if (eff.gt(5)) eff=eff.sqrt().mul(new Decimal(5).sqrt())
		eff=eff.add(tmp.f.coalEffect)
		return eff
	},
	effectDescription() {
		return "which makes point generation multiply itself by (log10(x+1)+1)^"+format(tmp.r.effect)
	},
	branches:['p'],
...
addLayer("f", {
	...
	milestones: {
		0: {
			requirementDescription: "3 Fuel",
			done() { return player.f.best.gte(3) || hasAchievement("a", 71) },
			effectDescription: "Keep B,G,R milestones on reset.",
		},
		1: {
			requirementDescription: "5 Fuel",
			done() { return player.f.best.gte(5) || hasAchievement("a", 71) },
			effectDescription: "Keep B,G,P upgrades on reset.",
		},
		2: {
			requirementDescription: "10 Fuel",
			done() { return player.f.best.gte(10) || hasAchievement("a", 71) },
			effectDescription: "You can max fuel.",
		},
	},
	buyables: {
		rows:3,
		...
		11: {
				...
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
                    cost=new Decimal(20).pow(x.div(new Decimal(1.2).pow(player[this.layer].buyables[33])).pow(2))
                    return cost.floor()
                },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
					let e = tmp.t.freeExtraTimeCapsules;
                    let display = (tmp.nerdMode?("Cost Formula: [can't be bothered with this, sorry]"):("Cost: " + formatWhole(tmp.f.buyables[11].cost) + " coal"))+"\n\
                    Amount: " + formatWhole(player[this.layer].buyables[this.id])+(e.gt(0)?(" + "+formatWhole(e)):"")+"\n\
					Effect: Multiplies coal gain by "+format(new Decimal(3).pow(player[this.layer].buyables[11]))+' before extreme mode reduction(3 per upgrade)'
					return display;
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
                    return player.f.coal.gte(tmp[this.layer].buyables[this.id].cost)},
                buy() { 
                    cost = tmp[this.layer].buyables[this.id].cost
                    player.f.coal = player.f.coal.sub(cost)	
                    player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].add(1)
                },
                buyMax() {
					if (!this.canAfford()) return;
					let b = player.f.coal.plus(1);
					let tempBuy = b.log(20).sqrt().max(0).floor();
		...
		12: {
				title: "Upgrade 2",
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
                    cost=new Decimal(100).pow(x.div(new Decimal(1.2).pow(player[this.layer].buyables[33])).add(1).pow(2))
                    return cost.floor()
                },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
					let e = tmp.t.freeExtraTimeCapsules;
                    let display = (tmp.nerdMode?("Cost Formula: [can't be bothered with this, sorry]"):("Cost: " + formatWhole(tmp.f.buyables[this.id].cost) + " coal"))+"\n\
                    Amount: " + formatWhole(player[this.layer].buyables[this.id])+(e.gt(0)?(" + "+formatWhole(e)):"")+"\n\
					Effect: Multiplies prestige point and point gain by "+format(new Decimal(2).pow(player[this.layer].buyables[this.id]))+' before extreme mode reduction(2 per upgrade)'
					return display;
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
                ...
                buyMax() {
					if (!this.canAfford()) return;
					let b = player.f.coal.plus(1);
					let tempBuy = b.log(100).sqrt().sub(1).max(0).floor();
					//if (tempBuy.gte(25) && tmp[this.layer].buyables[this.id].costScalingEnabled) tempBuy = tempBuy.times(25).sqrt();
					let target = tempBuy.plus(1).floor();
					player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].max(target);
				},
		...
		13: {
				title: "Upgrade 3",
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
                    cost=new Decimal(1500).pow(x.div(new Decimal(1.2).pow(player[this.layer].buyables[33])).add(1).pow(2))
                    return cost.floor()
                },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
                    ...
                    let display = (tmp.nerdMode?("Cost Formula: [can't be bothered with this, sorry]"):("Cost: " + formatWhole(tmp.f.buyables[this.id].cost) + " coal"))+"\n\
                    Amount: " + formatWhole(player[this.layer].buyables[this.id])+(e.gt(0)?(" + "+formatWhole(e)):"")+"\n\
					Effect: Multiplies rocket gain by "+format(new Decimal(2).pow(player[this.layer].buyables[this.id]))+' before extreme mode reduction(2 per upgrade)'
					return display;
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
                ...
                buyMax() {
					if (!this.canAfford()) return;
					let b = player.f.coal.plus(1);
					let tempBuy = b.log(1500).sqrt().sub(1).max(0).floor();
					//if (tempBuy.gte(25) && tmp[this.layer].buyables[this.id].costScalingEnabled) tempBuy = tempBuy.times(25).sqrt();
					let target = tempBuy.plus(1).floor();
					player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].max(target);
				},
		...
		33: {
				title: "Blue flame",
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
                    cost=new Decimal(1e5).mul(new Decimal(10).pow(new Decimal(2).pow(x)))
                    return cost.floor()
                },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
                    ...
                    let display = (tmp.nerdMode?("Cost Formula: [can't be bothered with this, sorry]"):("Cost: " + formatWhole(tmp.f.buyables[this.id].cost) + " coal"))+"\n\
                    Amount: " + formatWhole(player[this.layer].buyables[this.id])+(e.gt(0)?(" + "+formatWhole(e)):"")+"\n\
					Effect: reduces effective f buyables in cost formula by 1/"+format(new Decimal(1.2).pow(player[this.layer].buyables[this.id]))+'(1.2 per upgrade)\n\
					Note:Buying this resets all previous furnace progress'
					return display;
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
	...
	update(diff) {
		player.f.coal=player.f.coal.add(tmp.f.coalGain.mul(diff))
	},
	coalGain() {
		return new Decimal(2).pow(player.f.points).sub(1).mul(new Decimal(3).pow(player[this.layer].buyables[11])).pow(0.9)
	},
	effect() {
		let r=player.f.points.sqrt()
		return r.div(10)
	},
	coalEffect() {
		let eff=player.f.coal.plus(1).log10().pow(0.6).div(5);
	...
	tabFormat: {
		...
		"Furnace": {
			content: ['main-display', ['raw-html',()=>{return format(player.f.coal)+' coal('+format(tmp.f.coalGain)+'/s), which adds '+format(tmp.f.coalEffect)+' to the rocket effect'}], 'buyables']
		},
	},
	effectDescription() {
		return "which makes rocket effect "+format(tmp.f.effect.mul(100))+"% better, and add "+format(tmp.f.effect.mul(10))+" rockets to the effect"
	},
	branches:['r'],
    row: 2, // Row the layer is in on the tree (0 is the first row)
    hotkeys: [
        {key: "f", description: "F: Reset for fuel", onPress(){if (canReset(this.layer)) doReset(this.layer)}},
    ],
...
addLayer("b", {
        ...
        type: "static", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
		branches: ["p"],
        exponent() { return (((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?0.75:1.25)*(1/0.9) }, // Prestige currency exponent
		...
		effectBase() {
			...
			// MULTIPLY
			if (hasUpgrade('b',15)) base=base.mul(tmp.f.effect.add(1))
		...
		upgrades: {
			...
			15: {
				title: "Fueled boosters",
				description: "Booster base is multiplied by rocket fuel's first effect.",
				cost() { return tmp.h.costMult11b.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1438:31) },
				unlocked() { return player.t.unlocked },
				effectDisplay() { return format(tmp.b.upgrades[11].effect)+"x" },
				formula: "[should be obvious]"
			},
			21: {
				title: "Gen Z^2",
...
addLayer("g", {
        ...
        type: "static", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
		branches: ["p"],
        exponent() { return (((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.1:1.25)*(1/0.9) }, // Prestige currency exponent
		...
		effect() {
			...
			if (hasMilestone('r',1)) eff=eff.times(eff.add(1).log10().add(1).pow(tmp.r.effect))
			return eff.pow(0.9);
		},
		effectDescription() {
...
addLayer("t", {
        ...
        baseAmount() {return player.points}, // Get the current amount of baseResource
        type: "static", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return (((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?new Decimal(1.4):new Decimal(1.85)).mul((1/0.9)) }, // Prestige currency exponent
		...
		effect() { 
			if (!unl(this.layer)) return {gain: new Decimal(0), limit: new Decimal(0)};
				...
				gain: Decimal.pow(tmp.t.effBaseMult.times(tmp.t.effGainBaseMult).times(3).pow(tmp.t.effBasePow), player.t.points.plus(player.t.buyables[11]).plus(tmp.t.freeExtraTimeCapsules)).sub(1).max(0).times(player.t.points.plus(player.t.buyables[11]).gt(0)?1:0).times(tmp.t.enGainMult).mul(hasUpgrade('t',16)?(()=>{try {return tmp.t.effect.gain.add(1).log10().add(1).pow(tmp.r.effect.sqrt())} catch (err) {return new Decimal(1)}})():1).max(0).pow(0.9),
				limit: Decimal.pow(tmp.t.effBaseMult.times(tmp.t.effLimBaseMult).times(2).pow(tmp.t.effBasePow), player.t.points.plus(player.t.buyables[11]).plus(tmp.t.freeExtraTimeCapsules)).sub(1).max(0).times(100).times(player.t.points.plus(player.t.buyables[11]).gt(0)?1:0).times(tmp.t.enCapMult).mul(hasUpgrade('t',16)?(()=>{try {return tmp.t.effect.limit.add(1).log10().add(1).pow(tmp.r.effect.sqrt())} catch (err) {return new Decimal(1)}})():1).max(0).pow(0.9),
			}
		},
		effect2() {
		...
		upgrades: {
			...
			16: {
				title: "Burning time",
				description: "Rocket effect affects time energy cap and generation at a reduced rate.",
				cost() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?750:3) },
				unlocked() { return player.t.unlocked },
				effectDisplay() { return format(tmp.g.upgrades[11].effect)+"x" },
				formula() { return 'rocket effect applies but is square rooted' },
			},
			21: {
...
addLayer("e", {
        ...
        baseAmount() {return player.points}, // Get the current amount of baseResource
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?.025:.02).mul(0.9) }, // Prestige currency exponent
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1)
...
addLayer("s", {
        ...
        baseAmount() {return player.points}, // Get the current amount of baseResource
        type: "static", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.4:1.85).mul(1/0.9) }, // Prestige currency exponent
...
addLayer("sb", {
        ...
        type: "static", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
		branches: ["b"],
        exponent() { return (((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.075:1.25)*(1/0.9) }, // Prestige currency exponent
...
addLayer("sg", {
        ...
        type: "static", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
		branches: ["g"],
        exponent() { return (((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.225:1.25)*(1/0.9) }, // Prestige currency exponent
...
addLayer("h", {
        ...
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?.2:.125).mul(0.9) }, // Prestige currency exponent
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1)
...
addLayer("q", {
        ...
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?.008:.0075).mul(0.9) }, // Prestige currency exponent
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1)
...
addLayer("ss", {
        ...
        type: "static", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.07:1.1).mul(1/0.9) }, // Prestige currency exponent
...
addLayer("m", {
        ...
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?0.0085:0.007).pow(0.9) }, // Prestige currency exponent
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1);
...
addLayer("ba", {
        ...
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?0.0125:0.005).pow(0.9) }, // Prestige currency exponent
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1);
			if (hasAchievement("a", 74)) mult = mult.times(challengeEffect("h", 32));
...
addLayer("ps", {
        ...
        type: "static", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent: new Decimal(1.5*(1/0.9)), // Prestige currency exponent
		base() { 
...
addLayer("hn", {
		...
		exp() { return {m: new Decimal(0.025).mul(0.9), ba: new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?0.05:0.02).mul(0.9)} },
...
addLayer("n", {
        ...
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?0.05:0.03).mul(0.9) }, // Prestige currency exponent
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1);
...
addLayer("i", {
        ...
        type: "static", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent: new Decimal(1.8*(1/0.9)), // Prestige currency exponent
...
addLayer("ma", {
		...
		roundUpCost: true,
        type: "static", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent: new Decimal(1.1*(1/0.9)), // Prestige currency exponent
...
addLayer("ge", {
        ...
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent: new Decimal(0.01*0.9), // Prestige currency exponent
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1);
...
addLayer("mc", {
        ...
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent: new Decimal(4*0.9), // Prestige currency exponent
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1);
			if (player.mc.upgrades.includes(11)) mult = mult.times(buyableEffect("mc", 12));
            return mult
