...
function getResetGain(layer, useType = null) {
	...
	if (type=="static") {
		...
		let gain = tmp[layer].baseAmount.div(tmp[layer].requires).pow(tmp[layer].exponent).times(tmp[layer].gainMult.pow(0.9)).pow(tmp[layer].gainExp)
...
function getNextAt(layer, canMax=false, useType = null) {
	...
	if (type=="static") 
	{
		...
		next = next.root(tmp[layer].gainExp).div(tmp[layer].gainMult.pow(0.9)).root(tmp[layer].exponent).times(tmp[layer].requires).max(tmp[layer].requires)
