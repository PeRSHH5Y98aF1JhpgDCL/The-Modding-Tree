...
var thingsThatNeedToBeUpdated={
	innerHTML:{
		modinfo:()=>{return modInfo.name+" "+VERSION.withoutName},
		wintime:()=>{return `It took you ${formatTime(player.timePlayed)} to beat the game.`},
	},
}
var thingsThatNeedToBeUpdatedStyle={
	display:{
		endTab:()=>{return !(gameEnded && !player.keepGoing)?"none":"block"}
	}
}
var interval = setInterval(function() {
	...
	ticking = false
	Object.keys(thingsThatNeedToBeUpdated).forEach((x)=>{
		Object.keys(thingsThatNeedToBeUpdated[x]).forEach((y)=>{
			document.getElementById(y)[x]=thingsThatNeedToBeUpdated[x][y]()
		})
	})
	Object.keys(thingsThatNeedToBeUpdatedStyle).forEach((x)=>{
		Object.keys(thingsThatNeedToBeUpdatedStyle[x]).forEach((y)=>{
			document.getElementById(y).style[x]=thingsThatNeedToBeUpdatedStyle[x][y]()
		})
	})
}, 50)
