  ...
  function () {
    ...
    Decimal.prototype.fromDecimal = function (value) {
      if (value instanceof BDecimal) {
		  this.r=value
		  this.i=new BDecimal(0)
	  }
      return this;
    };

    Decimal.prototype.fromNumber = function (value) {
      this.r=new BDecimal(value)
	  this.i=new BDecimal(0)
      return this;
    };
    ...
    Decimal.prototype.fromString = function (value) {
	  ...
	  if (temp[temp.length-1]=='i') {
		  if (temp.split('+').length>1) {
			  ...
			  tempi.shift()
			  this.i=new BDecimal(tempi.join('+').slice(0, -1))
			  this.r=new BDecimal(temp.split('+')[0])
		  } else if(temp.split('-').length>1) {
			  let tempi=temp.split('-')
			  tempi.shift()
			  this.i=new BDecimal(tempi.join('-').slice(0, -1)).neg()
			  this.r=new BDecimal(temp.split('-')[0])
		  } else {
			  this.i=new BDecimal(temp.slice(0, -1))
			  this.r=new BDecimal(0)
		  }
	  } else {
		 this.i=new BDecimal(0)
		 this.r=new BDecimal(temp)
	  }
      return this;
    };

    Decimal.prototype.fromValue = function (value) {
      if (value instanceof Decimal||value instanceof BDecimal) {
    ...
    Decimal.prototype.mantissaWithDecimalPlaces = function (places) {
	  ...
	  if (this.i.eq(0)) return this.r.mantissaWithDecimalPlaces(places)
      return NaN//no
    };
    
    Decimal.prototype.magnitudeWithDecimalPlaces = function (places) {
      // https://stackoverflow.com/a/37425022
	  if (this.i.eq(0)) return this.r.magnitudeWithDecimalPlaces(places)
      return NaN//no
    };
    
    Decimal.prototype.toString = function () {
      if (this.i.eq(new BDecimal(0))) {
		  return this.r+''
	  } else {
		  return this.r+(this.i.lt(0)?'':'+')+this.i+'i'
	  }
    };
    ...
    Decimal.prototype.toStringWithDecimalPlaces = function (places) {
      if (this.i.eq(0)) {
		  return this.r.toStringWithDecimalPlaces(places)
	  } else {
		  return this.r.toStringWithDecimalPlaces(places)+(this.i.lt(0)?'':'+')+this.i.toStringWithDecimalPlaces(places)+'i'
	  }
    };
    
    Decimal.prototype.abs = function () {
	  let temp=new Decimal(0)
	  temp.r=this.r.pow(2).add(this.i.pow(2)).sqrt()
	  temp.i=new BDecimal(0)
      return temp
    };
    ...
    Decimal.prototype.add = function (value) {
      let temp=new Decimal(value)
	  let tempii=new Decimal(0)
	  if (!this.r) return new Decimal(NaN)
	  tempii.r=this.r.add(temp.r)
	  tempii.i=this.i.add(temp.i)
	  return tempii
    };
    ...
    Decimal.prototype.mul = function (value) {
      let temp=new Decimal(0)
	  let tempii=new Decimal(value)
	  if (!tempii.i) return new Decimal(NaN)
	  temp.r=this.r.mul(tempii.r).sub(this.i.mul(tempii.i))
	  temp.i=this.i.mul(tempii.r).add(tempii.i.mul(this.r))
      return temp;
    };
    ...
    Decimal.prototype.recip = function () {
		if (!this.r) return new Decimal(NaN)
      let temp=new Decimal(0)
	  let conj=this.r.pow(2).add(this.r.pow(2))
	  temp.r=temp.r.div(conj)
	  temp.i=temp.i.neg().div(conj)
	  return temp
    };
     ...
     */
    Decimal.prototype.quadrant = function () {
      let icomp=(!this.i.gt(0)?2:0)
	  let rcomp=(this.r.gt(0)?1:0)
	  if (icomp<1) rcomp=1-rcomp
	  let tot=icomp+rcomp+1
	  return tot//i'm just going to completely ignore edge cases
    };
	
	Decimal.prototype.quadrantRel = function (x) {
	  let temp=new Decimal(0)
	  temp.r=this.r.sub(x.r)
	  temp.i=this.i.sub(x.i)
      let icomp=(!this.i.gt(0)?2:0)
	  let rcomp=(this.r.gt(0)?1:0)
	  if (icomp<1) rcomp=1-rcomp
	  let tot=icomp+rcomp+1
    ...
    Decimal.prototype.eq = function (value) {
      var decimal = D(value);
      return this.r.eq(decimal.r)&&this.i.eq(decimal.i)
    };
    ...
    Decimal.prototype.lt = function (value) {
      ...
      var decimal = D(value);
	  if (decimal.i.eq(0)||this.i.eq(0)) return this.r.lt(decimal.r)
      return false
    };
    ...
    Decimal.prototype.gt = function (value) {
	  if (!this.r) return false
      var decimal = D(value);
	  if (decimal.i.eq(0)||this.i.eq(0)) return this.r.gt(decimal.r)
      return false
    };
